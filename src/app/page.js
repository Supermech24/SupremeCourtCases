...
const movementColors = {
  ...
  "Black Freedom Movement": "red",
  "Criminal Rights": "purple",
  "Other": "gray",
  "Womens Rights": "pink",
  "LGBTQ+ Rights": "orange"
};
...
export default function Home() {
  return (
    <div id="main-page">
      ...
      <div className="grid grid-cols-4 gap-4" id="case-grid">
        {supremeCourtCases.map((caseItem) => (
          <div className="p-4 flex flex-col items-center justify-around" id="case-box">
            <div
              className="movement-label"
              style={{ backgroundColor: movementColors[caseItem.movement] }}
            >
              {caseItem.movement}
            </div>
            <Link key={caseItem.id} id="case-desc" href={`/case/${caseItem.id}`} passHref>
              <div className="font-bold">{caseItem.name} ({caseItem.year})</div>
              <div>{caseItem.description}</div>
            </Link>
            <div id="liberty-box">
              <div id="liberty-ranking">Ed's liberty ranking:</div>
              <div id="slider-box">
                <div id="emoji-box">
                  <div>⛓️</div>
                  <div>🦅</div>
                </div>
                <Slider defaultValue={caseItem.libertyRank} disabled={false} />
              </div>
            </div>
          </div>
        ))}
